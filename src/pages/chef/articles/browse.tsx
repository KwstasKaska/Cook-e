...
export default function ChefBrowseArticlesPage() {
  ...
  return <ChefBrowseArticlesContent />;
}

const ChefBrowseArticlesContent = () => {
  ...
  const allChefArticles = chefData?.chefArticles ?? [];
  const chefArticles = allChefArticles.filter(
    (a) => a.creator?.id !== myUserId,
  );
  const chefHasMore = allChefArticles.length === LIMIT;
  ...
  const onPrev = () => {
    if (tab === 'chefs') {
      setChefOffset((o) => o - LIMIT);
    } else {
      setNutrOffset((o) => o - LIMIT);
    }
